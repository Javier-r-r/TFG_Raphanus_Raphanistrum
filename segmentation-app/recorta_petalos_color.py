...
for img_path in Path(input_folder).glob("*.tif"):
    ...
    # Limpieza básica
    mask = cv2.morphologyEx(mask_yellow, cv2.MORPH_OPEN, np.ones((7,7), np.uint8))
    ...
    for label in range(1, num_labels):
        petal_mask = (labels == label).astype(np.uint8) * 255
        x, y, w, h = cv2.boundingRect(petal_mask)
        if w < 30 or h < 30:
        ...
        petal_crop = img[y0:y1, x0:x1]
        out_name = f"{img_path.stem}_petal{label}.png"
        cv2.imwrite(os.path.join(output_folder, out_name), petal_crop)
        # Recortar la misma región en la máscara y guardar
        petal_mask_crop = mask_img[y0:y1, x0:x1]
        out_mask_name = f"{img_path.stem}_petal{label}.png"
        cv2.imwrite(os.path.join(output_mask_folder, out_mask_name), petal_mask_crop)
